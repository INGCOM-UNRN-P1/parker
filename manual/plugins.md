---
title: "Guía de Extensión y Creación de Plugins: parker"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de parker"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-parker-plugins)=
# Guía de Extensión y Plugins: parker

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`parker`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-parker-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`parker` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para parker`.
- **Punto de Entrada Oficial**: `parker.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-parker-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para parker."""
    name = "custom_rule"
...
Para que `parker` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."parker.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `parker` la reconozca:
...
(manual-parker-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `parker` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = parker.ejecutar_analisis(
...
(manual-parker-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `parker` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar parker
        run: pip install -e ./parker
        
      - name: Ejecutar Auditoría
        run: parker check src/ include/ --json > reporte.json
````

---

(manual-parker-plugins-ejercicios)=
